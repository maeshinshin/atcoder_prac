...
func main() {
	...
	w := make(map[int64]map[int64]struct{})
	for i := int64(0); i < m; i++ {
		tmp := scanInt64() - 1
		if _, ok := w[tmp]; ok {
			w[tmp][scanInt64()-1] = struct{}{}
		} else {
			w[tmp] = make(map[int64]struct{})
			w[tmp][scanInt64()-1] = struct{}{}
		}
	}
	...
	for i := int64(0); i < n; i++ {
		...
		flag[i] = true
		ans++
		queue.PushBack(int(i))
		count := int64(0)
		for count < n && queue.Len() != 0 {
			from := int64(queue.Front().Value.(int))
			queue.Remove(queue.Front())
			for k := range w[from] {
				if !flag[k] {
					ans++
					queue.PushBack(int(k))
					flag[k] = true
				}
			}
		}
